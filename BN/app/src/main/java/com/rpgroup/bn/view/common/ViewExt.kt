package com.example.apple.marvelgallery.view.common
...
fun mMap(L1:List<PersonalPic>):List<PersonalPicAdapter>{
    return L1.map(::PersonalPicAdapter)
}

fun mMap2(L1:List<PersonalPic>):List<MessageAdapter>{
    return L1.map(::MessageAdapter)
}